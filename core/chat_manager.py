...
class ChatManager(QObject):
    def __init__(self, orchestrator, parent: Optional[QObject] = None):  # Removed type hint to avoid circular import
        ...
        # Set initial active chat backend and model based on constants
        ...
        else:  # Default to Gemini if not specified or unknown
            self._active_chat_backend_id = "gemini_chat_default"  # Ensure it's a valid key
    ...
    def initialize(self):
        ...
        def configure_core_backends():
            logger.info("ChatManager: Configuring core backends asynchronously...")

            # Ensure all primary backends used by the UI are configured initially
            # This includes the default chat, default specialized, and any others shown in dropdowns
            core_backends_to_configure = {
            ...
            # Determine the actually active chat backend and its prompt for initial configuration
            active_chat_config = {
                self._active_chat_backend_id: (self._active_chat_model_name, self._active_chat_personality_prompt)
            }
            # Merge, ensuring the explicitly active chat backend's settings are prioritized if it's also in core_backends
            # This handles cases where DEFAULT_CHAT_BACKEND_ID might differ from "gemini_chat_default" etc.
            merged_backends_to_configure = {**core_backends_to_configure, **active_chat_config}

            for backend_id, (model_name, system_prompt) in merged_backends_to_configure.items():
                logger.info(f"CM: Initializing core backend '{backend_id}' with model '{model_name}'")

                effective_system_prompt = system_prompt
                # Override system prompt for specialized generator
                if backend_id == constants.GENERATOR_BACKEND_ID:
                    effective_system_prompt = constants.CODER_AI_SYSTEM_PROMPT
                # For primary chat backends, ensure the active personality is used
                elif backend_id in ["gemini_chat_default", "ollama_chat_default", "gpt_chat_default"]:
                    effective_system_prompt = self._active_chat_personality_prompt

                self._configure_backend(backend_id, model_name, effective_system_prompt)

            self._check_rag_readiness_and_emit_status()

        QTimer.singleShot(250, configure_core_backends)  # Slightly reduced delay
    ...
    def _configure_backend(self, backend_id: str, model_name: str, system_prompt: Optional[str]):
        ...
        try:
            ...
            # System prompt is passed directly, no need to re-check here if it's for generator
            # The initialize method now handles passing the correct system prompt.
    ...
    def _detect_task_type(self, user_text: str, filename: str) -> str:
        ...
        api_patterns = [r'\b(api|endpoint|route|fastapi|flask|django|rest|http|get|post|put|delete)\b',
                        r'\b(server|web service|microservice|backend)\b']
        data_patterns = [r'\b(data|csv|json|xml|parse|process|analyze|transform|clean)\b',
                         r'\b(pandas|numpy|database|sql|query|etl)\b']
        ui_patterns = [r'\b(ui|interface|window|dialog|widget|button|form|gui)\b',
        ...
        for pattern in api_patterns:
            if re.search(pattern, user_lower): return 'api'
        for pattern in data_patterns:
            if re.search(pattern, user_lower): return 'data_processing'
        for pattern in ui_patterns:
            if re.search(pattern, user_lower): return 'ui'
        return 'general'
    ...
    def _handle_backend_reconfigured_event(self, backend_id: str, model_name: str, is_configured: bool,
        ...
        if is_active_chat:
            ...
            self._emit_status_update(status_msg, status_color, not is_configured)  # Temporary if error
        elif is_active_spec:
    ...
    async def _handle_normal_chat_async(self, user_msg_txt: str, image_data: List[Dict[str, Any]]):
        ...
        should_use_rag = self._rag_handler.should_perform_rag(user_msg_txt, self._is_rag_ready, self._is_rag_ready)
    ...
    def _get_current_project_directory(self) -> str:
        ...
        if user_projects_dir and os.path.exists(user_projects_dir): return user_projects_dir
    ...
    def get_upload_service(self) -> Optional[UploadService]:
        return self._upload_service  # Made Optional

    def get_rag_handler(self) -> Optional[RagHandler]:
        return self._rag_handler  # Made Optional
