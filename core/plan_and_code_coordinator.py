...
try:
    ...
    ChatMessage, USER_ROLE, MODEL_ROLE, SYSTEM_ROLE, ERROR_ROLE, MessageLoadingState = (type("Fallback", (object,), {}),) * 6 # type: ignore
...
class PlanAndCodeCoordinator(QObject):
    ...
    def _emit_system_message_to_chat(self, message_text: str, is_error: bool = False, request_id_ref: Optional[str] = None):
    ...
    def start_planning_sequence(self,
        ...
        if self._active_planning_request_id or self._active_coder_request_ids:
            ...
            self._event_bus.uiStatusUpdateGlobal.emit("Coordinator is already working on a task!", "#e5c07b", True, 3000)
            return
        ...
        self._log_comm("SEQ_START", f"Planning for query: '{user_query[:50]}...' (PlannerReqID: {self._active_planning_request_id})")
        self._event_bus.uiStatusUpdateGlobal.emit(f"Asking Planner ({planner_llm_model_name}) to create a plan...", "#61afef", False, 0)
        ...
        if self._current_project_id and self._current_session_id:
            ...
            planner_placeholder_msg = ChatMessage(
                ...
                metadata={"purpose": "plan_and_code_planner_placeholder", "pacc_request_id": self._active_planning_request_id}
            )
            self._event_bus.newMessageAddedToHistory.emit(
        ...
        self._backend_coordinator.start_llm_streaming_task(
            ...
            request_metadata={
                ...
                "pacc_request_id": self._active_planning_request_id, # Ensure PACC ID is passed for LLM response routing
        ...
        try:
            ...
            for pattern in files_planned_patterns:
                ...
                if files_planned_match:
                    ...
                    files_list_str = files_planned_match.group(1) if files_planned_match.groups() and files_planned_match.group(1) else files_planned_match.group(0)
                    break
            ...
            try:
                ...
                if not isinstance(parsed_list_candidate, list):
                    ...
                    return False
                self._parsed_files_list = [str(f).strip().replace("\\", "/") for f in parsed_list_candidate if isinstance(f, str) and f.strip()]
            except Exception as e_eval:
                logger.error(f"PACC: Error evaluating FILES_PLANNED list string '{files_list_str}': {e_eval}", exc_info=True)
            ...
            if missing_instructions_for_files:
                ...
                self._log_comm("Parser", f"Warning: Using fallback instructions for: {', '.join(missing_instructions_for_files)}")
    ...
    async def _dispatch_code_generation_tasks_async(self):
        if not self._parsed_files_list or not self._coder_instructions_map:
            ...
            self._emit_system_message_to_chat("[System: Planner did not specify files or instructions. Cannot generate code.]", is_error=True)
            self._event_bus.uiStatusUpdateGlobal.emit("No files or instructions from plan to generate.", "#e5c07b", True, 4000)
        ...
        self._emit_system_message_to_chat(f"[System: Starting code generation for {len(self._parsed_files_list)} files...]")
        self._event_bus.uiStatusUpdateGlobal.emit(f"Sending {len(self._parsed_files_list)} file(s) to Code LLM...", "#61afef", False, 0)
        ...
        for filename in self._parsed_files_list:
            ...
            if not instructions or instructions.startswith("[Error:"): # Check for explicit error messages
                ...
                self._emit_system_message_to_chat(f"[System Error: Skipping code generation for `{filename}` due to missing/invalid instructions from planner.]", is_error=True)
                continue
    ...
    async def _generate_single_file_code_async(self, filename: str, instructions: str):
        if not self._specialized_llm_backend_id or not self._specialized_llm_model_name:
            ...
            self._emit_system_message_to_chat(f"[System Error: Code LLM not configured. Cannot generate `{filename}`.]", is_error=True)
            ...
            if not self._active_coder_request_ids and filename in self._parsed_files_list and self._parsed_files_list.index(filename) == len(self._parsed_files_list) -1 :
        ...
        if self._current_project_id and self._current_session_id:
            ...
            coder_placeholder_msg = ChatMessage(
                ...
                loading_state=MessageLoadingState.LOADING, # type: ignore
                metadata={"purpose": "plan_and_code_coder_placeholder", "filename": filename, "pacc_request_id": coder_request_id}
            )
            self._event_bus.newMessageAddedToHistory.emit(
                self._current_project_id,
                self._current_session_id,
    ...
    def _handle_all_coder_tasks_done(self):
        ...
        if self._active_coder_request_ids: # Should be empty if truly all done
            logger.warning(f"PACC: _handle_all_coder_tasks_done called, but {len(self._active_coder_request_ids)} tasks still marked active. This might be a race condition or error. Active IDs: {list(self._active_coder_request_ids.keys())}")
        ...
        for filename in self._parsed_files_list:
            ...
            if code and not err_msg: # Successfully generated code
                ...
                successfully_generated_files_for_validation.append(filename)
                 ...
                 self._emit_system_message_to_chat(f"[System Error: Code generation failed for `{filename}`: {err_msg[:100]}...]", is_error=True, request_id_ref=filename) # Use filename as a loose ref ID for UI
            else: # No code and no error message - implies task might not have run or had an unlogged issue
                 self._emit_system_message_to_chat(f"[System Error: Code generation for `{filename}` did not produce output or an error message.]", is_error=True, request_id_ref=filename)


        if successfully_generated_files_for_validation:
            self._log_comm("VALIDATE_START", f"Starting validation for {len(successfully_generated_files_for_validation)} files...")
            self._emit_system_message_to_chat(f"[System: Code generation complete. Starting validation for {len(successfully_generated_files_for_validation)} files...]")
            self._event_bus.uiStatusUpdateGlobal.emit(f"Code generated for {len(successfully_generated_files_for_validation)} files. Starting validation...", "#61afef", False, 0)
            ...
            self._emit_system_message_to_chat("[System: No files were successfully generated by Code LLM. Nothing to validate.]", is_error=True)
    ...
    def _handle_terminal_command_completed(self, command_id: str, exit_code: int, execution_time: float):
        ...
        logger.info(f"PACC: Validation command for '{filename}' (ID: {command_id}) completed with exit code {exit_code}.")
        ...
        if exit_code == 0:
            ...
            self._emit_system_message_to_chat(f"[System Error: Validation FAILED for `{filename}` (exit code {exit_code}). See LLM log for details. ❌]", is_error=True)
    ...
    def _handle_terminal_command_error(self, command_id: str, error_message: str):
        ...
        self._emit_system_message_to_chat(f"[System Error: Validation command for `{filename}` failed: {error_message[:100]}... ❌]", is_error=True)
        ...
        for filename in self._parsed_files_list: # Iterate over initially planned files
            ...
            if code and not err_msg: # Code exists and no error message (implies validation passed or not applicable)
                ...
                if code: # Code was generated but validation failed or other error
                     self._event_bus.modificationFileReadyForDisplay.emit(filename, f"# VALIDATION FAILED or ERROR: {final_error_msg}\n\n{code}")
                else: # No code was generated
                     self._event_bus.modificationFileReadyForDisplay.emit(filename, f"# CODE GENERATION FAILED: {final_error_msg}")


        summary_parts = [f"[System: Autonomous coding sequence for '{self._original_user_query[:50]}...' has finished."]
        if successful_files_final:
            summary_parts.append(f"Successfully generated/validated: {', '.join(f'`{f}`' for f in successful_files_final)}.")
        if failed_files_with_errors_final:
            failed_details = ", ".join([f"`{f}` ({failed_files_with_errors_final[f][:30]}...)" for f in failed_files_with_errors_final])
    ...
    def _handle_llm_responses(self, request_id: str, completed_message: ChatMessage, usage_stats_dict: dict):
        ...
        self._log_comm("LLM_RESP", f"Purpose: {purpose}, ReqID: {request_id}, For Planner: {request_id == self._active_planning_request_id}, For Coder: {request_id in self._active_coder_request_ids}")
        ...
        if purpose == "plan_and_code_planner" and request_id == self._active_planning_request_id:
            ...
            self._emit_system_message_to_chat(f"[System: Planner AI has provided a plan for '{self._original_user_query[:30]}...'. Parsing now...]", request_id_ref=request_id)
            ...
            if self._parse_planner_response(self._current_plan_text or ""):
                if not self._parsed_files_list:
                    ...
                    self._event_bus.uiStatusUpdateGlobal.emit("Planner indicates no files needed.", "#56b6c2", True, 3000)
                    ...
                    instructions_found_count = sum(1 for f in self._parsed_files_list if self._coder_instructions_map.get(f) and not self._coder_instructions_map.get(f, "").startswith("[Error:"))
                    ...
                    if all_instructions_ok:
                        ...
                        self._event_bus.uiStatusUpdateGlobal.emit("Plan parsed with errors. Code generation aborted.", "#e06c75", False, 0)
                        self._event_bus.uiInputBarBusyStateChanged.emit(False) # Release busy state
                        self._reset_sequence_state()
            else: # Parsing failed
            ...
            logger.debug(f"PACC: Raw code response for {filename} (length: {len(raw_code_response)} chars): '{raw_code_response[:100]}...'")
            ...
            if code_block_match:
                ...
                logger.warning(f"PACC: Could not extract fenced code block for '{filename}'. Storing raw response as code.")
                ...
                if len(raw_code_response) > 0 and ("def " in raw_code_response or "class " in raw_code_response or "import " in raw_code_response or raw_code_response.startswith("#")):
                     self._generated_code_map[filename] = (raw_code_response, "Warning: No fenced code block found, using raw response.")
                else:
                     self._generated_code_map[filename] = (None, "Error: No fenced code block found and raw response doesn't look like code.")
            ...
            self._emit_system_message_to_chat(final_coder_msg_text, is_error=(not extracted_code), request_id_ref=request_id)


            if request_id in self._active_coder_request_ids:
                del self._active_coder_request_ids[request_id]
            else: # Should not happen
                 logger.warning(f"PACC: Coder ReqID {request_id} for file {filename} was not in _active_coder_request_ids when trying to remove.")


            logger.info(f"PACC: Coder task for {filename} completed. Remaining active coder tasks: {len(self._active_coder_request_ids)}")
    ...
    def _handle_llm_errors(self, request_id: str, error_message: str):
        ...
        if request_id == self._active_planning_request_id:
            ...
            logger.info(f"PACC: Coder task for {filename} errored. Remaining active coder tasks: {len(self._active_coder_request_ids)}")
            if not self._active_coder_request_ids: # Check if all coder tasks are now done (including this error)
                logger.info("PACC: All coder tasks seem to be completed (with errors). Proceeding to _handle_all_coder_tasks_done.")
                self._handle_all_coder_tasks_done()
        else:
