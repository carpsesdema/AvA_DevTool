...
class BackendCoordinator(QObject):
    ...
    def get_available_models_for_backend(self, backend_id: str) -> List[str]:
        ...
        logger.debug(f"BC: get_available_models_for_backend called for {backend_id}")
        logger.debug(f"BC: Cached models for {backend_id}: {len(cached_models)} models")

        adapter = self._backend_adapters.get(backend_id)
        if not adapter:
            logger.warning(f"BC: No adapter found for {backend_id}")
        ...
        logger.debug(f"BC: Should refresh models for {backend_id}? {should_refresh}")

        if should_refresh and self._is_configured_map.get(backend_id, False):
            logger.debug(f"BC: Scheduling async model fetch for {backend_id}")
            self._schedule_async_model_fetch(backend_id)

        # REMOVED: The blocking "FORCE IMMEDIATE FETCH FOR DEBUGGING" section that was causing 30+ second freezes
